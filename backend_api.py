...
@app.get("/")
async def root():
    return {"status": "ok", "version": "6.5", "engine": "librosa" if HAS_LIBROSA else "fallback"}
...
def extract_melody_librosa(audio, sr):
    ...
    for i, of in enumerate(onset_frames):
        ...
        # Adaptive magnitude threshold: reject noise below 15% of peak
    max_mag = max((mags[f, t] for t in range(pitches.shape[1]) for f in range(pitches.shape[0]) if pitches[f, t] > 0), default=1.0)
    mag_threshold = max(0.02, max_mag * 0.15)

    if best_pitch > 0 and best_mag > mag_threshold:
    ...
    for e in melody_events:
        if e['midi'] != last_midi or (e['time'] - last_time) > 0.12:
            simplified.append(e)
            last_midi = e['midi']
            last_time = e['time']
    ...
    last_midi, last_time = -1, -1
    for e in all_voice:
        if e['midi'] != last_midi or (e['time'] - last_time) > 0.10:
...
def fit_piano_arrangement(melody_data, strategy='adaptive'):
    ...
    for i, e in enumerate(melody):
        ...
        dur = max(0.08, min(e['dur'], beat_dur * 2.0))  # shorter minimum, longer max
